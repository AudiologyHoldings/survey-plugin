...
class SurveyAnswerTestCase extends CakeTestCase {
	...
	function testFindReport(){
	  ...
	  $this->assertEqual('33.33%', $results['percent']['participation']);
	  $this->assertEqual('22.22%', $results['percent']['with_email']);
	  $this->assertEqual('33.33%', $results['percent']['completed_survey']);
	  $this->assertEqual('0%', $results['percent']['entered_give_away']);
	  $this->assertEqual('100%', $results['percent']['purchases']);
	  $this->assertEqual('100%', $results['percent']['oticon_purchases']);
	  $this->assertEqual('100%', $results['percent']['visit_clinic']);
